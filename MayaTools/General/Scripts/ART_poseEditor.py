...
class PoseEditor_UI():
	...
	if os.path.exists(settingsLocation):
	    ...
	    try:
		f = open(path + poseName + "__SelectionOnly.txt", 'wb')
	...
	cmds.confirmDialog(title = "Load Exact", message = "Loading exact")
	currentTime = cmds.currentTime(q = True)
	...
	if os.path.exists(path + poseName + ".txt"):
	    ...
	    cmds.confirmDialog(title = "Done loading", message = "Done Loading")
	    #key all
