...
def customMayaMenu():
	...
	cmds.menuItem(parent = customMenu, divider = True)
	cmds.menuItem(parent = customMenu, label = "FBX Export Batcher (Directory)", bld = False, enable = True, c = ART_fbxBatch)
	cmds.menuItem(parent = customMenu, label = "FBX Export Batcher (Scene)", bld = False, enable = True, c = ART_fbxSceneBatch)
...
#############################################################################################
#############################################################################################
def ART_fbxBatch(*args):
    import ART_fbxBatcher
    reload(ART_fbxBatcher)
    ART_fbxBatcher.FBX_Batcher()   
    
def ART_fbxSceneBatch(*args):
   
    import ART_fbxSceneBatch
    reload(ART_fbxSceneBatch)
    ART_fbxSceneBatch.multiCharacterFBX() 
